// File: vita3k/corey/src/app_init.cpp
// VITA3K_NO_GUI: Performance-optimized app initialization for Corey
...
void update_viewport(EmuEnvState &state) {
    // VITA3K_NO_GUI: Simplified viewport update for Corey
#ifdef COREY_BUILD
    if (corey::game_is_running.load(std::memory_order_relaxed)) {
        // Use cached values during gameplay to avoid recalculation
        if (state.viewport_cache_valid)
            return;
    }
#endif
    ...
    // VITA3K_NO_GUI: Fixed DPI scale for performance
#ifdef COREY_BUILD
    ...
    // VITA3K_NO_GUI: Skip font level updates during gameplay
#ifdef COREY_BUILD
    if (!corey::game_is_running.load(std::memory_order_relaxed)) {
#endif
    ...
    state.viewport_cache_valid = true;
#endif
}
...
static float fetch_x11_display_dpi() {
    // VITA3K_NO_GUI: Skip DPI detection for Corey
#ifdef COREY_BUILD
    return 1.0f;
#else
    int dpi = 96;
...
bool init(EmuEnvState &state, Config &cfg, const Root &root_paths) {
    ...
    // VITA3K_NO_GUI: Skip high DPI for Corey
#ifdef COREY_BUILD
    state.manual_dpi_scale = 1.0f;
#else
#ifdef __linux__
    ...
    // VITA3K_NO_GUI: Basic window setup for Corey
#ifdef COREY_BUILD
    Uint32 window_flags = window_type | SDL_WINDOW_RESIZABLE;
...
#else
    Uint32 window_flags = window_type | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI;
#endif
    ...
    state.window = WindowPtr(SDL_CreateWindow(
        ...
        SDL_WINDOWPOS_CENTERED, 
        DEFAULT_RES_WIDTH * state.manual_dpi_scale, 
        DEFAULT_RES_HEIGHT * state.manual_dpi_scale, 
    ...
    // VITA3K_NO_GUI: Additional Windows optimizations for Corey
#ifdef COREY_BUILD
    // Disable transparency and animations
    BOOL disable = TRUE;
    DwmSetWindowAttribute(wm_info.info.win.window, DWMWA_TRANSITIONS_FORCEDISABLED, &disable, sizeof(disable));
#endif
    ...
    if (!state.cfg.console) {
        if (renderer::init(state.window.get(), state.renderer, state.backend_renderer, state.cfg, root_paths)) {
            ...
            // VITA3K_NO_GUI: Set VSync based on game state for Corey
#ifdef COREY_BUILD
            if (state.backend_renderer == renderer::Backend::OpenGL) {
                // Start with VSync on, disable when game starts
