...
// VITA3K_USE_COREY: Include performance header
#include <corey/corey_perf.h>

namespace gui {

// VITA3K_USE_COREY: Optimized info message - skip if game running
void draw_info_message(GuiState &gui, EmuEnvState &emuenv) {
    COREY_SKIP_IF_GAME_RUNNING();
    ...
    if (emuenv.io.title_id.empty() && emuenv.cfg.display_info_message) {
        ...
        // VITA3K_USE_COREY: Disable window background and decorations
        ImGui::SetNextWindowPos(ImVec2(emuenv.logical_viewport_pos.x, emuenv.logical_viewport_pos.y), ImGuiCond_Always);
        ImGui::SetNextWindowSize(display_size, ImGuiCond_Always);
        ImGui::Begin("##information", nullptr, ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);
        ...
        ImGui::BeginChild("##info", WINDOW_SIZE, ImGuiChildFlags_Borders, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoResize);
...
// VITA3K_USE_COREY: Optimized style initialization - called once
static void init_style(EmuEnvState &emuenv) {
    ImGui::StyleColorsDark();

    ImGuiStyle *style = &ImGui::GetStyle();

    // VITA3K_USE_COREY: Minimal style settings for performance
    style->WindowPadding = ImVec2(11, 11);
    style->WindowRounding = 0.0f; // Disable rounding for performance
    style->FramePadding = ImVec2(4, 4);
    style->FrameRounding = 0.0f; // Disable rounding for performance
    style->ItemSpacing = ImVec2(10, 5);
    style->ItemInnerSpacing = ImVec2(6, 5);
    style->IndentSpacing = 20.0f;
    style->ScrollbarSize = 12.0f;
    style->ScrollbarRounding = 0.0f; // Disable rounding for performance
    style->GrabMinSize = 4.0f;
    style->GrabRounding = 0.0f; // Disable rounding for performance
    
    // VITA3K_USE_COREY: Disable transparency effects
    style->Alpha = 1.0f;
    style->DisabledAlpha = 0.6f;

    style->ScaleAllSizes(emuenv.manual_dpi_scale);

    // VITA3K_USE_COREY: Simplified color scheme with no transparency
    style->Colors[ImGuiCol_Text] = ImVec4(0.95f, 0.95f, 0.95f, 1.00f);
    style->Colors[ImGuiCol_TextDisabled] = ImVec4(0.24f, 0.23f, 0.29f, 1.00f);
    style->Colors[ImGuiCol_WindowBg] = ImVec4(0.07f, 0.08f, 0.10f, 1.00f); // No transparency
    style->Colors[ImGuiCol_ChildBg] = ImVec4(0.15f, 0.16f, 0.18f, 1.00f);
    style->Colors[ImGuiCol_PopupBg] = ImVec4(0.15f, 0.16f, 0.18f, 1.00f);
    style->Colors[ImGuiCol_Border] = ImVec4(0.80f, 0.80f, 0.80f, 1.00f); // No transparency
    style->Colors[ImGuiCol_BorderShadow] = ImVec4(0.92f, 0.91f, 0.88f, 0.00f);
    style->Colors[ImGuiCol_FrameBg] = ImVec4(0.10f, 0.09f, 0.12f, 1.00f); // No transparency
    style->Colors[ImGuiCol_FrameBgHovered] = ImVec4(0.24f, 0.23f, 0.29f, 1.00f); // No transparency
    style->Colors[ImGuiCol_FrameBgActive] = ImVec4(0.56f, 0.56f, 0.58f, 1.00f); // No transparency
    style->Colors[ImGuiCol_TitleBg] = ImVec4(0.10f, 0.09f, 0.12f, 1.00f);
    style->Colors[ImGuiCol_TitleBgCollapsed] = ImVec4(1.00f, 0.98f, 0.95f, 1.00f); // No transparency
    style->Colors[ImGuiCol_TitleBgActive] = ImVec4(0.07f, 0.07f, 0.09f, 1.00f);
    style->Colors[ImGuiCol_MenuBarBg] = ImVec4(0.10f, 0.09f, 0.12f, 1.00f);
    style->Colors[ImGuiCol_ScrollbarBg] = ImVec4(0.10f, 0.09f, 0.12f, 1.00f); // No transparency
    style->Colors[ImGuiCol_ScrollbarGrab] = ImVec4(0.80f, 0.80f, 0.83f, 1.00f); // No transparency
    ...
    style->Colors[ImGuiCol_ButtonHovered] = ImVec4(0.08f, 0.66f, 0.87f, 1.00f); // No transparency
    style->Colors[ImGuiCol_ButtonActive] = ImVec4(0.08f, 0.66f, 0.87f, 1.00f);
    style->Colors[ImGuiCol_Header] = ImVec4(1.00f, 1.00f, 0.00f, 1.00f); // No transparency
    style->Colors[ImGuiCol_HeaderHovered] = ImVec4(1.00f, 1.00f, 0.00f, 1.00f); // No transparency
    style->Colors[ImGuiCol_HeaderActive] = ImVec4(1.00f, 1.00f, 0.00f, 1.00f); // No transparency
    style->Colors[ImGuiCol_Separator] = ImVec4(0.10f, 0.10f, 0.10f, 1.00f);
    style->Colors[ImGuiCol_SeparatorHovered] = ImVec4(0.24f, 0.23f, 0.29f, 1.00f);
    style->Colors[ImGuiCol_SeparatorActive] = ImVec4(0.56f, 0.56f, 0.58f, 1.00f);
    style->Colors[ImGuiCol_ResizeGrip] = ImVec4(0.18f, 0.18f, 0.18f, 0.00f); // Disable resize grip
    style->Colors[ImGuiCol_ResizeGripHovered] = ImVec4(0.22f, 0.22f, 0.22f, 0.00f); // Disable resize grip
    style->Colors[ImGuiCol_ResizeGripActive] = ImVec4(0.32f, 0.32f, 0.32f, 0.00f); // Disable resize grip
    style->Colors[ImGuiCol_Tab] = ImVec4(0.80f, 0.80f, 0.83f, 1.00f); // No transparency
    ...
    style->Colors[ImGuiCol_PlotHistogram] = ImVec4(0.40f, 0.39f, 0.38f, 1.00f); // No transparency
    style->Colors[ImGuiCol_PlotHistogramHovered] = ImVec4(0.25f, 1.00f, 0.00f, 1.00f);
    style->Colors[ImGuiCol_TextSelectedBg] = ImVec4(1.00f, 1.00f, 0.00f, 1.00f); // No transparency
    style->Colors[ImGuiCol_ModalWindowDimBg] = ImVec4(1.00f, 0.98f, 0.95f, 1.00f); // No transparency
}

// Font initialization remains the same as it's called once
static void init_font(GuiState &gui, EmuEnvState &emuenv) {
    // ... (keep original font initialization code as it's only called once)
...
// Keep all other functions unchanged but add performance checks where appropriate
...
void init_app_background(GuiState &gui, EmuEnvState &emuenv, const std::string &app_path) {
    // VITA3K_USE_COREY: Skip background loading if game is running
    if (::corey::game_is_running.load(std::memory_order_relaxed))
        return;
...
// VITA3K_USE_COREY: Cache date/time calculations
std::map<DateTime, std::string> get_date_time(GuiState &gui, EmuEnvState &emuenv, const tm &date_time) {
   static std::map<DateTime, std::string> cached_date_time;
   static tm cached_tm = {};
   static uint32_t cached_date_format = 0;
   static uint32_t cached_time_format = 0;
   
   // Check if we need to recalculate
   if (cached_tm.tm_year == date_time.tm_year &&
       cached_tm.tm_mon == date_time.tm_mon &&
       cached_tm.tm_mday == date_time.tm_mday &&
       cached_tm.tm_hour == date_time.tm_hour &&
       cached_tm.tm_min == date_time.tm_min &&
       cached_date_format == emuenv.cfg.sys_date_format &&
       cached_time_format == emuenv.cfg.sys_time_format) {
       return cached_date_time;
   }
   
   cached_tm = date_time;
   cached_date_format = emuenv.cfg.sys_date_format;
   cached_time_format = emuenv.cfg.sys_time_format;
   cached_date_time.clear();
   ...
   if (!emuenv.io.user_id.empty()) {
       ...
       case SCE_SYSTEM_PARAM_DATE_FORMAT_YYYYMMDD:
           cached_date_time[DateTime::DATE_DETAIL] = fmt::format("{} {} ({})", month_str, days_str, day_str);
           cached_date_time[DateTime::DATE_MINI] = fmt::format("{}/{}/{}", year, month, day);
           break;
       case SCE_SYSTEM_PARAM_DATE_FORMAT_DDMMYYYY: {
           ...
           cached_date_time[DateTime::DATE_DETAIL] = fmt::format("{} {} ({})", small_days_str, small_month_str, day_str);
           cached_date_time[DateTime::DATE_MINI] = fmt::format("{}/{}/{}", day, month, year);
           break;
       }
       case SCE_SYSTEM_PARAM_DATE_FORMAT_MMDDYYYY:
           cached_date_time[DateTime::DATE_DETAIL] = fmt::format("{} {} ({})", month_str, days_str, day_str);
           cached_date_time[DateTime::DATE_MINI] = fmt::format("{}/{}/{}", month, day, year);
           break;
       }
   }
   ...
   if (clock_12h && date_time.tm_hour == 0)
       cached_date_time[DateTime::HOUR] = std::to_string(12);
   else
       cached_date_time[DateTime::HOUR] = std::to_string(clock_12h && date_time.tm_hour > 12 ? (date_time.tm_hour - 12) : date_time.tm_hour);

   cached_date_time[DateTime::CLOCK] = fmt::format("{}:{:0>2d}", cached_date_time[DateTime::HOUR], date_time.tm_min);
   cached_date_time[DateTime::DAY_MOMENT] = date_time.tm_hour >= 12 ? "PM" : "AM";
...
void pre_init(GuiState &gui, EmuEnvState &emuenv) {
   ...
   // VITA3K_USE_COREY: Disable ImGui window manipulation
   ImGuiIO &io = ImGui::GetIO();
...
void draw_begin(GuiState &gui, EmuEnvState &emuenv) {
   ...
   // VITA3K_USE_COREY: Update game running state
   ::corey::game_is_running.store(!emuenv.io.title_id.empty(), std::memory_order_relaxed);
}
...
// VITA3K_USE_COREY: Skip touchpad cursor if game is running
...
// VITA3K_USE_COREY: Optimized vita area drawing
void draw_vita_area(GuiState &gui, EmuEnvState &emuenv) {
   // Skip most UI if game is running
   if (::corey::game_is_running.load(std::memory_order_relaxed) && ::corey::skip_heavy_ui.load(std::memory_order_relaxed)) {
       // Only draw essential overlays
       ...
       if ((emuenv.cfg.show_info_bar || !emuenv.display.imgui_render) && gui.vita_area.information_bar)
...
// VITA3K_USE_COREY: Optimized UI drawing
void draw_ui(GuiState &gui, EmuEnvState &emuenv) {
   // Skip non-essential UI if game is running
   if (::corey::game_is_running.load(std::memory_order_relaxed) && ::corey::skip_heavy_ui.load(std::memory_order_relaxed))
       return;
       
   ImGui::PushFont(gui.vita_font[emuenv.current_font_level]);
   
   // Main menu bar is already disabled in main_menubar.cpp
