...
#else
// VITA3K_NO_GUI: STB function stubs for texture replacement
static unsigned char* stbi_load(const char *filename, int *x, int *y, int *comp, int req_comp) {
    LOG_WARN_ONCE("Texture replacement not supported in GUI-free build");
    return nullptr;
}
...
static int stbi_write_png(const char *filename, int w, int h, int comp, const void *data, int stride_in_bytes) {
    LOG_WARN_ONCE("Texture export not supported in GUI-free build");
    return 0;
}
#endif
...
#ifdef VITA3K_NO_GUI
    // VITA3K_NO_GUI: Texture replacement not supported
    if (import_textures || export_textures) {
        LOG_WARN_ONCE("Texture replacement/export not supported in GUI-free build");
    }
    this->import_textures = false;
    this->export_textures = false;
    this->save_as_png = false;
    return;
#endif
...
#ifdef VITA3K_NO_GUI
    // VITA3K_NO_GUI: Export not supported
    return;
#endif
...
#ifdef VITA3K_NO_GUI
    // VITA3K_NO_GUI: Import not supported
    return false;
#endif
